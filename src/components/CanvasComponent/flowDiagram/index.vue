...
export default {
    ...
    dagInfo: {
      handler() {
        ...
        if (!this.canvas || !this.canvas.inited) {
          ...
          this.component.dagInfo = this.flowData
        }
        this.toSetting()
      },
      deep: true
  ...
  methods: {
    initing() {
      const can = document.getElementById('flowDiagramCanvas')
      if (!can) {
        return false
      }
      const that = this
    ...
    drawComp(canvasDom) {
      ...
      that.canvas = new Layer.CanvasUtil(
        canvasDom,
        diagramOperation,
        (can) => {
    ...
    checkInfo() {
      ...
      for (const item of this.dagInfo.component_list) {
        ...
        final[item.component_name] = { status: status.charAt(0).toUpperCase() + status.slice(1), time: item.time }
      }
    ...
    toSetting() {
      if (this.component) {
        for (const key in this.dagCheck) {
          const props = (flowDiagram.RUNNING.match(this.dagCheck[key].status.toUpperCase()))
